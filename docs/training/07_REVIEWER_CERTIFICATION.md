...
| --- | --- |
| Rol | Reviewer |
| Nivel | Beginner / Reviewer |
| Duración | 12 horas |
| Objetivo | Formar revisores que comentan, validan y recomiendan correcciones antes de aprobación. |
| Prerrequisitos | Conocer proceso operativo que revisará. |
...
El `Reviewer` es un perfil formal de Compliance 360 Academy. Su entrenamiento está diseñado para que pueda usar la plataforma sin revisar código fuente, entendiendo módulos, permisos, responsabilidades, riesgos y límites reales del producto.

## ¿Qué responsabilidades tiene?

| Responsabilidad | Dueño | Prioridad | Evidencia esperada |
| --- | --- | --- | --- |
| Revisar contenido | Reviewer | Alta | Evidencia en Audit Trail / reporte / registro |
| Comentar hallazgos | Reviewer | Alta | Evidencia en Audit Trail / reporte / registro |
| Solicitar ajustes | Reviewer | Alta | Evidencia en Audit Trail / reporte / registro |
| Validar completitud | Reviewer | Alta | Evidencia en Audit Trail / reporte / registro |
| Escalar inconsistencias | Reviewer | Alta | Evidencia en Audit Trail / reporte / registro |
...
- Solicitar ajustes
- Validar completitud
- Escalar inconsistencias

## ¿Qué no puede hacer?

- Aprobar formalmente si no tiene permiso
- Cerrar CAPA
- Cambiar roles
- Ignorar criterios
...
```mermaid
flowchart TD
    Login[Login + MFA] --> Dashboard[Dashboard Reviewer]
    Dashboard --> S1[Revisar asignaciones]
    S1 --> S2[Leer documentos]
    S2 --> S3[Validar evidencia]
    S3 --> S4[Registrar comentarios]
    S4 --> S5[Escalar bloqueos]
    S5 --> Evidence[Audit Trail y evidencia]
    Evidence --> Outcome[Resultado controlado para Reviewer]
```
...
| Proceso | Reviewer | Tenant Admin | Quality Manager | Support Engineer | Consultora Admin |
| --- | --- | --- | --- | --- | --- |
| Revisar documento | R/A | I | I | C | C |
| Comentar CAPA | R/A | I | I | C | C |
| Revisar riesgo | R/A | I | I | C | C |
| Validar KPI | R/A | I | I | C | C |
| Consultar audit trail | R/A | I | I | C | C |
| Solicitar corrección | R/A | I | I | C | C |
...
```mermaid
flowchart LR
    R[Reviewer] --> A[Accountable en responsabilidades asignadas]
...
Preparar el acceso y el entorno de trabajo del rol `Reviewer` para operar sin fricción.
...
| --- | --- | --- |
| Revisar asignaciones | Diario | Validar resultado en dashboard/audit trail |
| Leer documentos | Diario | Validar resultado en dashboard/audit trail |
| Validar evidencia | Diario | Validar resultado en dashboard/audit trail |
| Registrar comentarios | Diario | Validar resultado en dashboard/audit trail |
| Escalar bloqueos | Diario | Validar resultado en dashboard/audit trail |
...
| --- | --- | --- |
| Revisiones pendientes | Monitorear tendencia | Escalar desviaciones |
| Comentarios abiertos | Monitorear tendencia | Escalar desviaciones |
| Correcciones solicitadas | Monitorear tendencia | Escalar desviaciones |
| Tiempo de revisión | Monitorear tendencia | Escalar desviaciones |
| Evidencias faltantes | Monitorear tendencia | Escalar desviaciones |

---

# CAPÍTULO 5 - Procesos Paso a Paso

### 5.1 Revisar documento

**Objetivo:** ejecutar el proceso `Revisar documento` de forma trazable, segura y alineada con el rol `Reviewer`.
...
**Objetivo:** ejecutar el proceso `Comentar CAPA` de forma trazable, segura y alineada con el rol `Reviewer`.
...
1. Iniciar sesión en Compliance 360.
2. Confirmar tenant, rol activo y permisos visibles.
3. Abrir el módulo relacionado con `Comentar CAPA`.
4. Revisar dashboard, filtros y estado actual antes de modificar información.
5. Crear, actualizar, aprobar, consultar o ejecutar la acción permitida por el rol.
6. Adjuntar evidencia cuando el proceso lo requiera.
7. Validar resultado esperado y registrar observaciones.
...
**Objetivo:** ejecutar el proceso `Revisar riesgo` de forma trazable, segura y alineada con el rol `Reviewer`.
...
**Objetivo:** ejecutar el proceso `Validar KPI` de forma trazable, segura y alineada con el rol `Reviewer`.
...
1. Iniciar sesión en Compliance 360.
2. Confirmar tenant, rol activo y permisos visibles.
3. Abrir el módulo relacionado con `Validar KPI`.
4. Revisar dashboard, filtros y estado actual antes de modificar información.
5. Crear, actualizar, aprobar, consultar o ejecutar la acción permitida por el rol.
6. Adjuntar evidencia cuando el proceso lo requiera.
7. Validar resultado esperado y registrar observaciones.
...
**Objetivo:** ejecutar el proceso `Consultar audit trail` de forma trazable, segura y alineada con el rol `Reviewer`.
...
**Objetivo:** ejecutar el proceso `Solicitar corrección` de forma trazable, segura y alineada con el rol `Reviewer`.
...
1. Iniciar sesión en Compliance 360.
2. Confirmar tenant, rol activo y permisos visibles.
3. Abrir el módulo relacionado con `Solicitar corrección`.
4. Revisar dashboard, filtros y estado actual antes de modificar información.
5. Crear, actualizar, aprobar, consultar o ejecutar la acción permitida por el rol.
6. Adjuntar evidencia cuando el proceso lo requiera.
7. Validar resultado esperado y registrar observaciones.
...
### 6.1 Escenario: Documento con errores

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `Documento con errores` dentro de un tenant productivo o de capacitación.
...
### 6.2 Escenario: CAPA incompleta

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `CAPA incompleta` dentro de un tenant productivo o de capacitación.
...
### 6.3 Escenario: Riesgo sin control

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `Riesgo sin control` dentro de un tenant productivo o de capacitación.
...
### 6.4 Escenario: KPI sin fórmula

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `KPI sin fórmula` dentro de un tenant productivo o de capacitación.
...
### 6.5 Escenario: Evidencia ilegible

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `Evidencia ilegible` dentro de un tenant productivo o de capacitación.
...
### 6.6 Escenario: Revisión cruzada

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `Revisión cruzada` dentro de un tenant productivo o de capacitación.
...
### 6.7 Escenario: Cambio regulatorio

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `Cambio regulatorio` dentro de un tenant productivo o de capacitación.
...
### 6.8 Escenario: Comentario no atendido

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `Comentario no atendido` dentro de un tenant productivo o de capacitación.
...
### 6.9 Escenario: Revisión urgente

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `Revisión urgente` dentro de un tenant productivo o de capacitación.
...
### 6.10 Escenario: Reporte de inconsistencias

**Contexto empresarial:** el rol `Reviewer` enfrenta el caso `Reporte de inconsistencias` dentro de un tenant productivo o de capacitación.
...
**Situación:** el rol `Reviewer` debe trabajar con `Document Management` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `Workflow Engine` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `CAPA Management` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `Risk Management` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `Quality Indicators` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `Audit Trail` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `Revisar documento` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `Comentar CAPA` en Compliance 360.

A. Prometer funcionalidad no implementada para Comentar CAPA sin aclaración.
B. Compartir credenciales para acelerar el trabajo sobre Comentar CAPA.
C. Omitir Audit Trail porque el proceso Comentar CAPA es interno.
D. Validar permisos, tenant, evidencia y estado antes de operar Comentar CAPA.
...
**Situación:** el rol `Reviewer` debe trabajar con `Revisar riesgo` en Compliance 360.

A. Validar permisos, tenant, evidencia y estado antes de operar Revisar riesgo.
...
**Situación:** el rol `Reviewer` debe trabajar con `Validar KPI` en Compliance 360.

A. Compartir credenciales para acelerar el trabajo sobre Validar KPI.
B. Validar permisos, tenant, evidencia y estado antes de operar Validar KPI.
C. Omitir Audit Trail porque el proceso Validar KPI es interno.
D. Prometer funcionalidad no implementada para Validar KPI sin aclaración.
...
**Situación:** el rol `Reviewer` debe trabajar con `Consultar audit trail` en Compliance 360.

A. Omitir Audit Trail porque el proceso Consultar audit trail es interno.
B. Compartir credenciales para acelerar el trabajo sobre Consultar audit trail.
C. Validar permisos, tenant, evidencia y estado antes de operar Consultar audit trail.
...
**Situación:** el rol `Reviewer` debe trabajar con `Solicitar corrección` en Compliance 360.

A. Prometer funcionalidad no implementada para Solicitar corrección sin aclaración.
B. Compartir credenciales para acelerar el trabajo sobre Solicitar corrección.
C. Omitir Audit Trail porque el proceso Solicitar corrección es interno.
D. Validar permisos, tenant, evidencia y estado antes de operar Solicitar corrección.
...
**Situación:** el rol `Reviewer` debe trabajar con `RBAC` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `MFA` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `Estado real del módulo` en Compliance 360.
...
**Situación:** el rol `Reviewer` debe trabajar con `Buenas prácticas ISO` en Compliance 360.
...
A. Prometer funcionalidad no implementada para Revisar documento sin aclaración.
B. Compartir credenciales para acelerar el trabajo sobre Revisar documento.
C. Omitir Audit Trail porque el proceso Revisar documento es interno.
D. Validar permisos, tenant, evidencia y estado antes de operar Revisar documento.
...
A. Validar permisos, tenant, evidencia y estado antes de operar Comentar CAPA.
B. Compartir credenciales para acelerar el trabajo sobre Comentar CAPA.
C. Omitir Audit Trail porque el proceso Comentar CAPA es interno.
D. Prometer funcionalidad no implementada para Comentar CAPA sin aclaración.
...
A. Omitir Audit Trail porque el proceso Validar KPI es interno.
B. Compartir credenciales para acelerar el trabajo sobre Validar KPI.
C. Validar permisos, tenant, evidencia y estado antes de operar Validar KPI.
D. Prometer funcionalidad no implementada para Validar KPI sin aclaración.
...
A. Prometer funcionalidad no implementada para Consultar audit trail sin aclaración.
B. Compartir credenciales para acelerar el trabajo sobre Consultar audit trail.
C. Omitir Audit Trail porque el proceso Consultar audit trail es interno.
D. Validar permisos, tenant, evidencia y estado antes de operar Consultar audit trail.
...
A. Validar permisos, tenant, evidencia y estado antes de operar Solicitar corrección.
B. Compartir credenciales para acelerar el trabajo sobre Solicitar corrección.
C. Omitir Audit Trail porque el proceso Solicitar corrección es interno.
D. Prometer funcionalidad no implementada para Solicitar corrección sin aclaración.
...
A. Validar permisos, tenant, evidencia y estado antes de operar Revisar documento.
B. Compartir credenciales para acelerar el trabajo sobre Revisar documento.
C. Omitir Audit Trail porque el proceso Revisar documento es interno.
D. Prometer funcionalidad no implementada para Revisar documento sin aclaración.
...
A. Compartir credenciales para acelerar el trabajo sobre Comentar CAPA.
B. Validar permisos, tenant, evidencia y estado antes de operar Comentar CAPA.
C. Omitir Audit Trail porque el proceso Comentar CAPA es interno.
D. Prometer funcionalidad no implementada para Comentar CAPA sin aclaración.
...
A. Prometer funcionalidad no implementada para Validar KPI sin aclaración.
B. Compartir credenciales para acelerar el trabajo sobre Validar KPI.
C. Omitir Audit Trail porque el proceso Validar KPI es interno.
D. Validar permisos, tenant, evidencia y estado antes de operar Validar KPI.
...
A. Validar permisos, tenant, evidencia y estado antes de operar Consultar audit trail.
B. Compartir credenciales para acelerar el trabajo sobre Consultar audit trail.
C. Omitir Audit Trail porque el proceso Consultar audit trail es interno.
D. Prometer funcionalidad no implementada para Consultar audit trail sin aclaración.
...
A. Compartir credenciales para acelerar el trabajo sobre Solicitar corrección.
B. Validar permisos, tenant, evidencia y estado antes de operar Solicitar corrección.
C. Omitir Audit Trail porque el proceso Solicitar corrección es interno.
D. Prometer funcionalidad no implementada para Solicitar corrección sin aclaración.
...
### 11.2 Ejercicio práctico: Comentar CAPA

**Caso:** ejecutar `Comentar CAPA` en un tenant de entrenamiento llamado `Academy Demo Tenant`.
...
### 11.3 Ejercicio práctico: Revisar riesgo
...
### 11.4 Ejercicio práctico: Validar KPI

**Caso:** ejecutar `Validar KPI` en un tenant de entrenamiento llamado `Academy Demo Tenant`.
...
### 11.6 Ejercicio práctico: Solicitar corrección

**Caso:** ejecutar `Solicitar corrección` en un tenant de entrenamiento llamado `Academy Demo Tenant`.
